...
async function checkReply() {
    ...
    console.log(dateTime, ' : check Reply (', countercheck, '), Msgqueue = ', msgqueue, 'process=', processed);

    if (msgqueue.length > 0 && isConnected) {
...
function processMessage(chat,images) {
    ...
    sleeps(1000).then(async () => {
        ...
        var data = { idchat: "", message: "" ,images:""};
        var d =
            data.idchat = from;
        data.message = body;
        data.images = images;
        ...
        switch (args[0].toLowerCase()) {
            ...
            default:
               // console.log("encoded=", ms);
                ...
                fetchJson(webHook + '/sendHook.php?source=' + ms)
                    .then(response => {
                        ...
                        try {
                            ...
                            if (datas.status) {
                                if (datas.response != "") {
                                   ...
                                   var number = from.split('@')[0];

                                   number = number.substr(1);
                                    
                                   send_message(datas.response,number,datas.images,false,false);
...
const sendMessageWTyping = async (msg, jid) => {
    ...
    if (isConnected) {
        ...
        if (exists?.jid || (exists && exists[0]?.jid)) {
            ...
            let jids = exists.jid || exists[0].jid;


            //await sock.presenceSubscribe(jids)
            //await delay(500)

            //await sock.sendPresenceUpdate('composings', jids)
            //await delay(2000)

            //console.log("Success")
            //await sock.sendPresenceUpdate('composing', jids)
            //await delay(500)
            ...
            sock.sendMessage(exists.jid || exists[0].jid, msg) // { text: pesankirim })
                .then((result) => {
                    ...
                    // });
                    console.log("Success")
                    //sock.sendPresenceUpdate('available', jids)
...
async function send_message(message,numbers,image,files,res){
   const req = { body : { message : message , 
                          number : numbers,
                          images : image},
                 files : files                           
               }
    ...
    try {
        if (!req.files) {
            if (!number) {
                ...
                console.log('send to Number=', numberWA);
                console.log(await sock.onWhatsApp(numberWA));
                if (isConnected) {
                    const exists = await sock.onWhatsApp(numberWA);
                    if (exists?.jid || (exists && exists[0]?.jid)) {

                        let jid = exists.jid || exists[0].jid;

                        await sock.sendPresenceUpdate('composing', jid)
                        await delay(2000)

                        var urls = 'https://lh3.googleusercontent.com/ogw/AF2bZyibt5M1ynNUsZ88Xmv2Gjzcpx6n3mSBqnqxF72m9OJVfus=s32-c-mo';
...
const buttons = [
  ...
  {buttonId: 'id3', buttonText: {displayText: 'Button 3'}, type: 1}
                        ...
                        buttonMessage = {
                             text: pesankirim,
                            //image: {url: images},
                          //  caption: pesankirim,//"Hi it's button message",
                           footerText: pesankirim,
...
buttonMessage = {
   ...
   buttonMessage = {
                        ...
                        buttonMessage =   { text: pesankirim };
                    }


                       // sock.sendMessage(exists.jid || exists[0].jid, { text: pesankirim })
                       sock.sendMessage(exists.jid || exists[0].jid, buttonMessage)
                            .then((result) => {

                                //setTimeout(function(){sock.sendPresenceUpdate('available', jid)}),200);
                        ...
                        console.log('Nomor ${number} tidak terdaftar.');
                        if (res){
                        res.status(500).json({
                            status: false,
                            response: `Nomor ${number} tidak terdaftar.`,
...
async function updateChatStatus(data) {
    ...
    fetch(webHook + '/updateHook.php?source=' + ms).then(response => {
        console.log(response.text());
    });
...
async function connectToWhatsApp() {
    ...
    sock.ev.on('messages.update', m => {
        ...
        {
            key: {
              remoteJid: '6289635866667:67@s.whatsapp.net',
              id: 'BAE528476F2C7E48',
              fromMe: true,
        ...
        //console.log("message update to:",m[0].update.status);

        if (m[0].update.status) {
            var msg = m[0];
            console.log("message update to:", m[0].update.status);

            if (msg.key.fromMe) {

                console.log("Message has been sent, status: ", msg.update.status, " id:", msg.key.id);
                uid = msg.key.id;
                const idchat = msg.key.remoteJid;
                var data = { id: id, ack: msg.update.status, chat_id: msg.key.id, idchat: idchat, message: "" }
                updateChatStatus(data);
    ...
    //sock.ev.on("messages.update", async ({ messages, type }) => {
    //    console.log("message update : data type=",type," data message=",messages);
    //});
    sock.ev.on("messages.upsert", async ({ messages, type }) => {
        console.log("Message upsert : data type=", type, " data message=", messages);
        ...
        if (messageType === 'imageMessage') {
            ...
            var ext = ".jpg";
            console.log('mime=', m.message.imageMessage.mimetype);
            if (mime == 'image/jpeg')
                ext = '.jpeg';
                ...
                for (const msg of messages) {
                    ...
                    if (msg.key.fromMe && type === "append" && msg.status) {
                        ...
                        if ( msg.message.extendedTextMessage)
                            msgg =  msg.message.extendedTextMessage.text;
                        else 
                           msgs = "";
                        var data = { id: id, ack: msg.status, chat_id: msg.key.id, idchat: idchat, message: msgg }
...
app.post("/send-message", async (req, res) => {
 ...
 send_message(req.body.message,req.body.number,req.body.images,req.files,res);
